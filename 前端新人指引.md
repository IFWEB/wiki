...
有时打包会出错，导致服务器挂掉。此时需要重启。  
 ...
 2. 进入到/home/ifweb/node-deploy 目录下
 ```
 $ cd /home/ifweb/node-deploy
 ```
 3. 重启服务
 ```
 $ sh ci/deploy.sh
...
#### mongodb数据库重启：
 ...
 ```
 $ cd /home/ifweb/mongodb-linux-x86_64-3.4.2/bin
 ```
 3. 杀掉进程：
 ```
 $ killall -9 mongod
...
#### redis数据库重启：
 1. 用ssh连接上10.1.60.68 账号／密码： root／nongfadai
 2. 进入到/home/ifweb/redis-stable/src 目录下
 ```
 $ cd /home/ifweb/redis-stable/src
 ```
 3. 重启redis
 ```
...
4. 执行打包命令: 运行node master -p 8002
5. 在浏览器访问localhost:8002就能看到新console首页了
6. 如需配置访问的服务器ip,定位到./build/node-dev/config/index.js中的ip配置,同上前端代码运行
